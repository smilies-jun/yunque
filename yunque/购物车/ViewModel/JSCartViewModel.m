...
- (void)getData{
    ...
    for (int i = 0; i<allCount; i++) {
        //创造店铺下商品数据
        NSInteger goodsCount = [_DataArray count];
        ...
        for (int x = 0; x<goodsCount; x++) {
            ...
            cartModel.p_id         = [[_DataArray objectAtIndex:x]objectForKey:@"cartId"];
            cartModel.p_product_id         = [[_DataArray objectAtIndex:x]objectForKey:@"cartDetailId"];
...
- (void)deleteGoodsBySingleSlide:(NSIndexPath *)path{
    ...
    if (shopArray.count == 0) {
        ...
        for (JSCartModel *model in shopArray) {
            _deleteShopId = model.p_product_id;
            if (model.isSelect) {
                isSelectShopCount++;
