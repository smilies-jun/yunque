...
- (void)lfb_GetPicture:(NSString *)title maxCount:(int)maxCount callBack:(LFBPhotoPickerCallback)callback{
    ...
    switch (self.isSelectedPhotoAlbum) {
        ...
        {
            UIActionSheet *actionSheet = [[UIActionSheet alloc]initWithTitle:title delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"拍摄",@"从相册中选择",@"从已有样式中添加", nil];
            [actionSheet setTag:LFBPhotoPickerStyleAllDevice];
            [actionSheet showInView:self.currentViewController.view];
        }
            break;
        case LFBPhotoPickerStyleAllOnlyDevice:
        {
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (actionSheet.tag == LFBPhotoPickerStyleAllDevice) {
        ...
        [actionSheet dismissWithClickedButtonIndex:0 animated:YES];
            ...
            NSString *tokenID = NSuserUse(@"Mystate");
            if ([tokenID integerValue]) {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"choseMen" object:nil];

            }else{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"chose" object:nil];
    ...
    }else if (actionSheet.tag == LFBPhotoPickerStyleAllOnlyDevice){
        if (buttonIndex == 0) {
            [self pickerServiceAccessPhotoWithCamera];
            [actionSheet dismissWithClickedButtonIndex:0 animated:YES];
        }else if (buttonIndex == 1){
