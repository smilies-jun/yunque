...
- (void)viewDidLoad {
   ...
   // NSLog(@"shop == %lu",[_shopArray count]);
    ...
    for (int i =0; i < _shopArray.count; i++) {
        JSCartModel *model = [_shopArray objectAtIndex:i];
        if (model.isSelect) {
...
- (void)payPostClick{
    ...
    for (int i =0 ; i< [buyArray count]; i++) {
        ...
        [shopArray addObject:model.p_product_id];
        NSDictionary *dic = [[NSDictionary alloc]initWithObjectsAndKeys:@(model.p_quantity),@"number", nil];
